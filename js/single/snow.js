...
const snow = new DOMParser().parseFromString(svg_str, 'text/xml').children[0]
...
const newSnow = () => {
	// create a new snow
	const clonedSnow = snow.cloneNode(true);

	// const duration = Math.max(10 * Math.random(), 5) * 1000;
	const left = Math.floor(document.body.offsetWidth * Math.random());
	const zIndex = Math.max(2 * Math.random(), 0.75)
	const duration = (10000 / (zIndex * 1.5));

	// add it to the dom
	document.body.append(clonedSnow)

	clonedSnow.style.left = left + 'px';
	clonedSnow.style.zIndex = zIndex;
	clonedSnow.style.transform = `scale(${zIndex})`

	// then animate it
	clonedSnow
		.animate(
			{ transform: `translateY(${window.innerHeight + 24}px) scale(${zIndex})` },
			{ duration, fill: 'forwards' })
		// garbage collection (when it's done animating, delete the element)
		.onfinish = anim => anim.target.effect.target.remove()
}
