...
a string to the `sayHello` function. JavaScript will happing execute that, but
...
development works. There is a neat little feature called
[Hot Module Replacement][hmr]. This will replace the changed modules in your
...
apps in a number of different languages (called bindings). And you can write GTK
...
where he would regenerate the typescript definition files regularly, and we
...
I knew that I had to start working on away to use types generated by this new
...
definition files from `gi-typescript-definitions` but now we will need to
generate them ourselves in Workbench, but
[there is currently an issue blocking this][issue-1], but hopefully it will be
resolved soon.

### Porting the demos

Workbench has a huge library of demos. And I would recommend you look there next
...
[porting all the JavaScript demos to TypeScript][pr-6], which is currently
ongoing.
...
JS demos for the price of mainting one code sample. Pretty neat if you ask me!

## Wrapping up

I hope you enjoy the TypeScript support in Workbench, and start writing GNOME
apps!! If you have any questions or feedback please reach out to me.

### Acknowledgements

I would like to thank [Google Summer of Code][gsoc] for providing me with the
opportunity of working on this, and the [GNOME Foundation][gnome] in general.
